# ClawTower v0.2.0 Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Ship one release addressing all 6 critical gaps — noise reduction, process attribution, update signing, network baseline, compiler false positives, and health heartbeat.
...
**Current version:** 0.1.9 → target 0.2.0
...
Expected: FAIL — a2 passes through because exact message differs.
...
The agent runs as a systemd service → auid=4294967295 (unset). Human SSH sessions have auid matching their login UID. Use this to tag alerts.
...
pub async fn send_heartbeat(&self, uptime_secs: u64, alert_count: u64) -> Result<()> {
    ...
    let payload = serde_json::json!({
        ...
        "username": "ClawTower",
        "icon_emoji": ":shield:",
        "text": format!("💚 ClawTower heartbeat — uptime: {}h {}m, alerts processed: {}",
...
fn verify_release_signature(binary_data: &[u8], sig_bytes: &[u8]) -> Result<()> {
    ...
    // Sign over SHA256 of binary (not raw binary — too large)
...
if let Some(sig_url) = sig_url {
    ...
    bail!("❌ Release has no .sig file — refusing to install unsigned binary");
}
```
...
Read from JSONL file, filter, return JSON array. Keep implementation simple — read last N lines, parse, filter.
...
Cover:
- Too many alerts → configure allowlist, check dedup settings
- Scan timeouts → increase timeout or disable slow scans
- Slack webhook not working → test with `clawtower test-webhook`
- High CPU usage → reduce scan frequency
- False positives from cargo build → explained and fixed in v0.2.0
...
git commit -m "docs: add configuration reference, troubleshooting, and changelog for v0.2.0"
```
...
```bash
git tag -a v0.2.0 -m "v0.2.0: noise reduction, attribution, signing, monitoring"
