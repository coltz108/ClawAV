# ClawTower Noise Fixes + LD_PRELOAD Detection — Design
...
3. **Create clawtower-deny sudoers** — Fix root cause of repeating immutable flag alerts.

### Policy YAML fixes (no Rust)
4. **Fix detect-scheduled-tasks** — Remove `"at "` from `command_contains` (matches `cat`).
5. **Fix deny-clawtower-config-write** — Only match write tools, not `cat`/`grep`.
