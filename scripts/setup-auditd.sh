...
# Write rules file
cat > /etc/audit/rules.d/openclawav.rules << EOF
# OpenClawAV — Monitor openclaw user activity
# Generated by setup-auditd.sh
# Architecture: $ARCH

# Monitor all process execution by openclaw
-a always,exit -F arch=b64 -S execve -F uid=$WATCHED_UID -k openclawav_exec

# Monitor file access to sensitive dirs
-w /etc/openclawav/ -p rwxa -k openclawav_tamper
-w /usr/local/bin/openclawav -p rwxa -k openclawav_tamper
-w /etc/systemd/system/openclawav.service -p rwxa -k openclawav_tamper

# Monitor privilege escalation attempts
-a always,exit -F arch=b64 -S setuid -S setgid -S setreuid -S setregid -F uid=$WATCHED_UID -k openclawav_privesc

# Monitor network socket creation
-a always,exit -F arch=b64 -S socket -S connect -F uid=$WATCHED_UID -k openclawav_net

# Monitor file permission changes (arch-specific syscalls)
-a always,exit -F arch=b64${PERM_S_FLAGS} -F uid=$WATCHED_UID -k openclawav_perm

# Monitor module loading attempts
-a always,exit -F arch=b64 -S init_module -S finit_module -k openclawav_module
EOF

# Reload rules
augenrules --load
echo "Auditd rules loaded. $(auditctl -l | grep -c openclawav) OpenClawAV rules active."
