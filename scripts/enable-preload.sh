#!/bin/bash
# Install libclawguard.so and configure LD_PRELOAD for OpenClawAV
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
LIB_DIR="/usr/local/lib/openclawav"
POLICY_DIR="/etc/openclawav"
POLICY_FILE="$POLICY_DIR/preload-policy.json"
SO_FILE="$LIB_DIR/libclawguard.so"
SERVICE_FILE="/etc/systemd/system/openclawav.service"
...
mkdir -p "$POLICY_DIR"
mkdir -p /var/log/openclawav

if [ ! -f "$POLICY_FILE" ]; then
    cat > "$POLICY_FILE" <<'POLICY'
{
  "deny_exec": ["bash -c", "sh -c"],
  "deny_paths_write": ["/etc/shadow", "/etc/passwd", "/etc/hosts", "/etc/openclawav"],
  "deny_connect": [],
...
if [ -f "$SERVICE_FILE" ]; then
    ...
    if ! grep -q "LD_PRELOAD" "$SERVICE_FILE"; then
        ...
        sed -i '/^\[Service\]/a Environment=LD_PRELOAD=/usr/local/lib/openclawav/libclawguard.so' "$SERVICE_FILE"
...
echo "  Log:     /var/log/openclawav/preload.log"
