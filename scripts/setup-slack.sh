#!/bin/bash
# Setup Slack webhook for ClawTower independent alerting
# This webhook must be SEPARATE from any OpenClaw Slack integration

set -euo pipefail

CONFIG_FILE="${1:-/etc/clawtower/config.toml}"

echo "╔══════════════════════════════════════════════════════════╗"
echo "║       ClawTower — Independent Slack Webhook Setup      ║"
echo "╚══════════════════════════════════════════════════════════╝"
echo ""
...
echo "  3. Name it 'ClawTower Watchdog' (or similar)"
...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$WEBHOOK_URL" \
    ...
    -d '{"text":"🛡️ ClawTower test message — webhook configured successfully!"}')
...
echo ""
echo "Done! ClawTower will now send alerts to Slack independently."
